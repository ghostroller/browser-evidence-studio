...
export class ProjectExecutions {
  ...
  async begin(input:{executionId:string;projectId:string;materialRevisionId:string;materialContentHash:string;directory:string;dependencyLockPath:string;input:unknown;mode:ExecutionMode;runId:string;pageId:string;environmentRef:string}):Promise<ManagedExecution>{
    ...
    return {binding,datasets,snapshotDirectory,
      ...
      saveStep:event=>save(async()=>{
        ...
        if(event.state==='running')await this.original(directory,`${name}-start.json`,{executionId:binding.executionId,attemptId:attempt,recordingId:input.runId,pageId:input.pageId,startedAt:new Date().toISOString()});
      ...
      close:async()=>{await tail;this.active.delete(binding.executionId);await datasets.close();},
    };
  }
  ...
  async assess(projectId:string,id:string,selected?:DatasetIdentity[]):Promise<ReportSummary>{
    ...
    const scopes=(await Promise.all([...new Set(identities.map(item=>item.attemptId))].map(attempt=>this.scope(directory,attempt)))).filter((item):item is AttemptScope=>!!item);
    ...
    const jsonSources=new CapturedJsonSourceReader(async sourceRef=>{
      let metadata;try{metadata=await reader.artifactMetadata(sourceRef);}catch{return;}
      const time=Date.parse(metadata.createdAt),sourcePage=typeof metadata.source==='object'?metadata.source?.pageId:undefined,matches=scopes.filter(scope=>scope.executionId===id&&scope.recordingId===value.runId&&sourcePage===scope.pageId&&time>=Date.parse(scope.startedAt)&&time<=Date.parse(scope.finishedAt!));
      if(matches.length!==1)return;
      return {reader,scope:{executionId:id,attemptId:matches[0].attemptId,recordingId:value.runId}};
    });
  ...
  async recordSample(projectId:string,id:string,attemptId:string,target:HistoricalElementRef):Promise<string>{
    const value=await this.state(projectId,id);ensure(this.active.has(id)&&!value.finishedAt&&target.position.recordingId===value.runId&&target.position.pageId===value.pageId,'Sample does not belong to the active execution recording',409);
    executionId(attemptId);
    const replay=await this.materials.replay(value.runId,projectId),node=await replay.node(target,{maxBytes:1024*1024,limit:1});
    ...
    await this.original(await this.directory(id),`sample-${sourceRef}.json`,sample);return sourceRef;
  }
  ...
  async reports(projectId:string,id:string,budget:ReadBudget){
    ...
    const result={...ids,items,returnedBytes:0};ensure(Buffer.byteLength(JSON.stringify(result))<=budget.maxBytes,'Report summaries exceed budget; request fewer items',413);result.returnedBytes=Buffer.byteLength(JSON.stringify(result));return result;
  }
