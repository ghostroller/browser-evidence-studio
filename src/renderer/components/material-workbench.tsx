...
export function MaterialWorkbench({ projectId, recordingId, position, selectedTarget, onOpenReplay, onSelectTarget }: {
  ...
  const refreshLists = useCallback(async () => {
    ...
    const [nextDrafts, nextRevisions] = await Promise.all([
      call('materialDrafts', { limit: 50, maxBytes: 24576 }), call('materialRevisions', { limit: 50, maxBytes: 24576 }),
    ]);
    if (token === request.current) { setDrafts(nextDrafts); setRevisions(nextRevisions); }
  }, [call]);
  const loadCollection = useCallback(async (selected: Draft, collection: Collection, cursor?: string) => {
    ...
    setPages(current => ({ ...current, [collection]: cursor ? { ...result, items: [...current[collection].items, ...result.items] } : result }));
  }, [call]);
  const openDraft = useCallback(async (id: string) => {
    const token = ++request.current;
    const selected: Draft = await call('materialDraft', { draftId: id });
    if (token !== request.current) return;
    setDraft(selected); setPages(empty()); setCardId(''); setConflict(null);
    await Promise.all(COLLECTIONS.map(collection => loadCollection(selected, collection)));
  }, [call, loadCollection]);
  useEffect(() => { ++request.current; setDraft(null); setPages(empty()); setError(''); void refreshLists().catch(failure => setError(String(failure))); }, [projectId, refreshLists]);
  useEffect(() => {
    if (!selectedTarget) return;
    setFieldTarget(selectedTarget);
  }, [selectedTarget]);
  const mutate = async (edits: Edit[], message: string) => {
    if (!draft || pending) return;
    setPending('保存资料'); setError(''); setConflict(null);
    try {
      const result = await call('editMaterialDraft', { draftId: draft.draftId, expectedDraftRevision: draft.draftRevision, edits });
      if (result.status === 'conflict') { setConflict(result.current); setError(`草稿已由其他操作更新：本地修订 ${draft.draftRevision}，当前修订 ${result.current.draftRevision}。请重新读取后再编辑。`); return; }
      setDraft(result.draft); setNotice(message);
      await Promise.all(COLLECTIONS.map(collection => loadCollection(result.draft, collection)));
  ...
  const allRecordingRefs = async () => {
    if (!draft) return [] as string[];
    let cursor: string | undefined;
    const result: string[] = [];
    do {
      const page: Page<string> = await call('materialCollection', { kind: 'draft', draftId: draft.draftId, collection: 'recordingRefs', cursor, limit: 100, maxBytes: 24576 });
  ...
  const saveCard = async () => {
    ...
    const refs = await allRecordingRefs();
    ...
    await mutate(edits, card ? '已更新 checkpoint 草稿。' : '已在历史位置新增 checkpoint 草稿。');
    setCardId(next.id);
  };
  const saveRequirement = async () => {
    ...
    await mutate([{ operation: 'upsert', collection: 'requirements', item: { id, description, dataset: selectedRequirement?.dataset,
      rules, fieldIds: selectedRequirement?.fieldIds ?? [] } satisfies MaterialRequirement }], '需求已保存到草稿。');
    setRequirementId(id);
  };
  const saveField = async () => {
    ...
    await mutate([{ operation: 'upsert', collection: 'fields', item: field }, { operation: 'upsert', collection: 'requirements', item: requirement }], '字段和需求关联已保存。');
    setFieldId(id);
  };
  const saveAnnotation = async () => {
    ...
    await mutate([{ operation: 'upsert', collection: 'annotations', item: annotation },
      { operation: 'upsert', collection: 'checkpoints', item: { ...card, annotationIds: [...card.annotationIds, id] } }], '历史元素注释已保存。');
    setAnnotationText('');
  };
  const publish = async () => {
    if (!draft || pending) return;
    ...
    try {
      const revision = await call('publishMaterialDraft', { draftId: draft.draftId, expectedDraftRevision: draft.draftRevision });
      setNotice(`已发布候选资料版本 ${revision.revisionId}；发布不表示人工验收通过。`);
      await openDraft(draft.draftId); await refreshLists();
    } catch (failure) { setError(String(failure)); }
    finally { setPending(''); }
  };
  return <div className="material-workbench">
    ...
    <div className="material-layout"><aside className="material-list"><div className="section-label">草稿 <Button disabled={!!pending} onClick={() => void call('createMaterialDraft').then((created: Draft) => openDraft(created.draftId)).then(refreshLists).catch(failure => setError(String(failure)))}>新建</Button></div>
      ...
      {drafts.nextCursor && <Button onClick={() => void call('materialDrafts', { cursor: drafts.nextCursor, limit: 50 }).then((next: Page<Draft>) => setDrafts({ ...next, items: [...drafts.items, ...next.items] })).catch(failure => setError(String(failure)))}>更多草稿</Button>}
      ...
      {revisions.nextCursor && <Button onClick={() => void call('materialRevisions', { cursor: revisions.nextCursor, limit: 50 }).then((next: Page<Revision>) => setRevisions({ ...next, items: [...revisions.items, ...next.items] })).catch(failure => setError(String(failure)))}>更多版本</Button>}
      ...
      {pages.checkpoints.nextCursor && <Button onClick={() => void loadCollection(draft, 'checkpoints', pages.checkpoints.nextCursor)}>下一页 checkpoint</Button>}
      ...
      <Button disabled={!selectedRequirement || !!pending} onClick={() => void saveField()}>保存字段</Button>{pages.fields.nextCursor && <Button onClick={() => void loadCollection(draft, 'fields', pages.fields.nextCursor)}>更多字段</Button>}
      </section></>}</div></div>
  </div>;
}
