...
import { TaskAuthorizations } from './components/task-authorizations';
...
export function App() {
  ...
  const [historicalTarget, setHistoricalTarget] = useState<SelectionReceipt | null>(null);
  const [selectionSession, setSelectionSession] = useState<{request:SelectionRequest;replayId?:string;generation?:number}|null>(null);
  ...
  useEffect(() => {
    ...
    if (!projectId) return;
    let cancelled = false;
    void window.studio.call('materialRevisions', { projectId, limit: 50, maxBytes: 24576 }).then(page => { if (!cancelled) setRevisionPage(page); }).catch(failure => { if (!cancelled) setError(`资料版本读取失败：${String(failure)}`); });
    return () => { cancelled = true; };
  }, [projectId]);
  useEffect(() => {
    if (panel !== 'validation' || !projectId) return;
    let cancelled = false;
    void window.studio.call('materialRevisions', { projectId, limit: 50, maxBytes: 24576 }).then(page => { if (!cancelled) setRevisionPage(page); }).catch(failure => { if (!cancelled) setError(`资料版本读取失败：${String(failure)}`); });
    return () => { cancelled = true; };
  }, [panel, projectId]);
  ...
  const openValidation = async (id: string) => {
    const indexed = items(state.validations).find((record: any) => record.id === id);
    if (indexed?.executionBinding || indexed?.executionId) { openFixedResults(indexed.executionId || id); return; }
    const selection = ++historySelection.current;
  ...
  const closeReplayWorkspace = useCallback(() => {
    setSelectingHistory(false); setSelectionSession(null);setReplayRecordingId(''); setRequestedReplayPosition(null); setReplayPosition(null); setReplayState(null); setHistoricalTarget(null);
  }, []);
  ...
  const receiveHistoricalTarget = useCallback((target: HistoricalElementRef,replayId:string,generation:number) => {
    if(!selectionSession || selectionSession.replayId!==replayId || selectionSession.generation!==generation ||
      selectionSession.request.projectId!==projectId || !sameReplayPosition(target.position,selectionSession.request.anchor))return;
    setHistoricalTarget({request:selectionSession.request,target,replayId,generation});setSelectionSession(null);setSelectingHistory(false);setPanel('materials');
  }, [selectionSession,projectId]);
  const cancelHistoricalSelection = useCallback(() => {setSelectionSession(null);setSelectingHistory(false);}, []);
  const beginHistoricalSelection = useCallback((request:SelectionRequest) => {
    if(request.projectId!==projectId || !request.checkpointId){setError('选择目标与当前资料项目不一致。');return;}
    setHistoricalTarget(null);setSelectionSession({request});setSelectingHistory(true);openReplayPosition(request.anchor);
    setNotice(request.purpose==='annotation'?'正在暂停并定位卡片历史页，以选择注释对象。':'正在暂停并定位卡片历史页，以绑定字段元素。');
  }, [projectId,openReplayPosition]);
  const selectionReady=useCallback((replayId:string,generation:number)=>{
    setSelectionSession(current=>current?{...current,replayId,generation}:current);
  },[]);
  ...
  return <BrowserPresentation browserBox={browserBox} revision={revision} overlay={overlay} onError={setError}><div className="app-shell">
    <header className="topbar" inert={selectingHistory}><div className="brand"><strong>Browser Evidence Studio</strong></div><div className="topbar-spacer" />
      ...
      <Separator orientation="vertical" className="topbar-separator" /><Button variant="ghost" disabled={!projectId} onClick={() => setOverlay('tasks')}>任务授权</Button><Button variant="ghost" onClick={() => setOverlay('setup')}><Settings />连接与环境</Button>
    </header>
...
<p className="hint">Agent 重复执行通过“任务授权”授予范围、时限与撤销；历史一次性启动授权不再提供。</p><p className="hint">{validationDisabledReason || '按所选项目和登录环境创建验收记录；已有示范会自动封存；起点采用地址栏 URL。'}</p></div>} second={<div className="pane-scroll form-stack"><div className="execution-actions"><Badge value={active?.execution || 'ready'} /><Button disabled={stopping || (!active && !state.validationStarting)} className="danger-quiet" onClick={() => perform(stopExecution)}>停止并接管</Button></div><Button className="full" disabled={!active || !!busy} onClick={() => perform(() => call('saveProfile', {}, '已刷新可持久化浏览器数据并登记登录环境；未承诺完整状态快照。'))}>保存当前登录环境</Button>{activeValidation && <div className="validation-result"><div className="section-label">本次执行</div><div className="execution-actions"><Badge value={activeValidation.status} /><Badge value={activeValidation.validation?.overall || 'not-run'} /></div><Button className="full" onClick={() => perform(() => openValidation(activeValidation.id))}>查看需求、数据与验收</Button></div>}<p className="hint">未覆盖、证据不足、机器失败与人工判定分别保存。修改脚本后须重新运行。</p></div>} />}
      ...
      <div className="evidence-strip"><div><span>{active ? `Run ${active.id.slice(0, 12)} · ${workspaceCheckpoints.length} 个已加载 checkpoint` : '连续录制与保存点分别记录'}</span></div><Button disabled={!workspaceRunId} onClick={() => perform(() => openHistory(workspaceRunId))}>打开证据时间线</Button><Button disabled={!workspaceRunId || !projectId} onClick={() => perform(() => openRecordingReplay(workspaceRunId))}>打开历史回放</Button></div>
        ...
        {replayRecordingId && <ReplayWorkspace projectId={projectId} recordingId={replayRecordingId} requestedPosition={requestedReplayPosition} selecting={selectingHistory} canStop={!!active || !!state.validationStarting} onPosition={receiveReplayPosition} onTarget={receiveHistoricalTarget} onSelectionReady={selectionReady} onCancelSelection={cancelHistoricalSelection} onStop={() => perform(async () => { cancelHistoricalSelection(); await stopExecution(); closeReplayWorkspace(); })} onClose={closeReplayWorkspace} onError={setError} />}
...
{!replayRecordingId && <div className="browser-toolbar">{liveSession && !active && <><span>实时页面 · 未录制</span><Button disabled={!!busy} onClick={() => perform(() => call('closeSession', {}, '已关闭浏览器会话。'))}>关闭浏览器会话</Button></>}<Button disabled={!selectedPage || !!busy || liveSession?.locked || liveSession?.controller !== 'human'} onClick={() => perform(() => call('closePage', { pageId: selectedPage?.pageId }))}>关闭当前页</Button><Button disabled={!active || !!busy || active.controller !== 'human'} onClick={() => perform(() => call('pauseOperations', { paused: !active?.locked }))}>{active?.locked ? '恢复人工输入' : '暂停页面输入'}</Button><Button disabled={!active || !!busy} onClick={() => perform(() => call('pauseCapture', { paused: active?.capture !== 'paused' }))}>{active?.capture === 'paused' ? '恢复录制' : '暂停录制'}</Button><Button disabled={!active || !!busy || ['running', 'waiting-human', 'finalizing'].includes(active.execution)} onClick={() => perform(() => call('control', { controller: active?.controller === 'agent' ? 'human' : 'agent' }, active?.controller === 'agent' ? '已收回人工控制。' : '已授予 Agent 控制；人工输入已锁定。'))}>{active?.controller === 'agent' ? '收回人工控制' : '交给 Agent 控制'}</Button><div className="topbar-spacer" /></div>}
    ...
    <Dialog open={!!overlay} onOpenChange={open => { if (!open) closeOverlay(); }}><DialogContent className={'overlay-panel ' + (['setup','recovery','project','profile'].includes(overlay || '') ? 'narrow' : '')} showCloseButton={false}>
      <div className="overlay-heading"><DialogTitle>{overlay === 'evidence' ? '证据与验收存档' : overlay === 'results' ? '固定版本结果中心' : overlay === 'tasks' ? '任务授权与撤销' : overlay === 'recovery' ? '检查与恢复存档' : overlay === 'project' ? '新建项目' : overlay === 'profile' ? '添加登录环境' : '连接与已验证环境'}</DialogTitle><Button onClick={closeOverlay}>返回工作台</Button></div>
      ...
      {overlay === 'results' && fixedExecutionId && <div className="overlay-body"><ResultCenter key={`${projectId}/${fixedExecutionId}`} projectId={projectId} executionId={fixedExecutionId} /></div>}
      {overlay === 'evidence' && <Tabs className="archive-layout" value={historyTab} onValueChange={value => {++artifactRequest.current;setHistoryTab(value);setArtifact(null);}}><div className="archive-meta"><code>{historyRunId}</code><span>有界读取；缺失、截断与真实空值分开</span><Button disabled={!historyRunId} onClick={() => perform(async () => { await openRecordingReplay(historyRunId); closeOverlay(); })}>回放此存档</Button></div><TabsList className="archive-tabs">{[['checkpoints', 'Checkpoint'], ['events', '时间线'], ['gaps', '缺口'], ['summary', '运行摘要'], ['validation', '结构化数据 / 验收']].map(([key, title]) => <TabsTrigger key={key} value={key} className={historyTab === key ? 'selected' : ''}>{title}</TabsTrigger>)}</TabsList><div className="overlay-body archive-body">
