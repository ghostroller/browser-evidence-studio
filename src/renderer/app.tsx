...
export function App() {
  ...
  return <div className="app-shell">
    ...
    {overlay && <div className="overlay"><div className={`overlay-panel ${['setup', 'recovery'].includes(overlay) ? 'narrow' : ''}`}><div className="overlay-heading"><div><p className="eyebrow">{overlay === 'evidence' ? 'EVIDENCE ARCHIVE' : overlay === 'replay' ? 'RECORDED DOM' : overlay === 'recovery' ? 'ARCHIVE RECOVERY' : 'LOCAL CONNECTION'}</p><h2>{overlay === 'evidence' ? '证据与验收存档' : overlay === 'replay' ? 'DOM 基础回放' : overlay === 'recovery' ? '检查与恢复存档' : '连接与已验证环境'}</h2></div><button onClick={() => { setOverlay(null); setArtifact(null); }}>返回工作台</button></div>
...
function RunRecoveryView({ runId, active, onRecovered, onOpen }: { runId: string; active: boolean; onRecovered(): Promise<any>; onOpen(): Promise<any> }) {
  ...
  const inspect = useCallback(async () => {
    const current = ++request.current; setPending('inspect'); setError(''); setData(null);
    try { const result = await window.studio.call('inspectRunRecovery', { runId }); if (current === request.current) setData(result); }
    catch (failure) { if (current === request.current) setError(String(failure)); }
    finally { if (current === request.current) setPending(''); }
  }, [runId]);
  useEffect(() => { void inspect(); return () => { request.current++; }; }, [inspect]);
  const recover = async () => {
    setPending('recover'); setError('');
    try { await window.studio.call('recoverRun', { runId, expectedFingerprint: data.inspection.lockFingerprint }); setRecovered(true); await onRecovered(); }
    catch (failure) { setError(String(failure)); setData(null); }
    finally { setPending(''); }
  };
  const states: Record<string, string> = { unlocked: '未见写入标记', live: '另一个写入进程仍在运行', dead: '原写入进程已退出', 'pid-reused': '进程编号已被其他进程使用', unknown: '尚不能确认原写入进程', corrupt: '写入标记损坏' };
  const state = data?.inspection?.state;
  return <div className="overlay-body run-recovery" data-run-id={runId}><code>{runId}</code><p>恢复只重建可读存档与验收记录。已有原件保留，旧浏览器控制权和执行栈不会恢复。</p>{data && <><h3 data-lock-state={state}>{states[state] || state}</h3><p>{data.inspection.message}</p>{data.inspection.owner?.pid && <p className="muted">原进程编号：{data.inspection.owner.pid} · 检查时间：{time(data.inspection.checkedAt)}</p>}{data.error && <details><summary>上次读取失败原因</summary><p>{data.error}</p></details>}</>}{pending && <p role="status">{pending === 'recover' ? '正在确认所有权并重建存档…' : '正在检查写入所有权…'}</p>}{error && <p className="error-inline" role="alert">{error}</p>}{recovered && <p className="notice">存档已恢复为可读状态。请检查已保存材料和中断原因，再新建运行重试。</p>}{active && <p className="notice">当前存在打开的运行；请先结束并封存，再恢复存档。</p>}{data && !data.canRecover && data.status === 'unreadable' && !active && <p className="hint">当前状态不能安全恢复。原件保持不变，可稍后重新检查。</p>}<div className="recovery-actions"><button disabled={!!pending} onClick={() => void inspect()}>重新检查</button>{!recovered && data?.canRecover && <button className="primary" disabled={!!pending || active} onClick={() => void recover()}>{state === 'unlocked' ? '重建可读索引' : '安全恢复存档'}</button>}{(recovered || data && data.status !== 'unreadable') && <button className="primary" disabled={!!pending} onClick={() => { void onOpen().catch(failure => setError(String(failure))); }}>查看已保存材料</button>}</div></div>;
}

function InterruptedValidationView({ record, checkpoints, historyError, active, onCheckpoints, onRetry }: { record: any; checkpoints: any[]; historyError?: string; active: boolean; onCheckpoints(): void; onRetry(): void }) {
  const interrupted = ['interrupted', 'failed', 'cancelled'].includes(record.status);
  return <section className="interrupted-validation"><div className="detail-title"><h3>{interrupted ? '执行未形成完整验收报告' : '尚无完整验收报告'}</h3><Badge value={record.status} /></div><p>{record.recovery?.reason || record.error || '当前没有可确认的完成报告，不能据此判定需求通过。'}</p>{record.error && record.error !== record.recovery?.reason && <p className="error-inline">{record.error}</p>}<p>运行：<code>{record.runId}</code></p><p>已保存的 checkpoint 和附件仍可单独查看；{historyError ? '本次材料读取失败，不能当作没有证据。' : `当前页已读取 ${checkpoints.length} 个 checkpoint。`}</p><p className="hint">重新执行会创建新的 run。旧控制权、人工等待和未完成的自动化步骤不会继续。</p><div className="recovery-actions"><button onClick={onCheckpoints}>查看已有 checkpoint</button><button className="primary" disabled={active || !interrupted || !record.projectId} onClick={onRetry}>准备新运行重试</button></div>{active && <p className="hint">先结束并封存当前运行，再选择新运行的登录环境与输入。</p>}</section>;
}
