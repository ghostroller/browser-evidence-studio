...
import { JsonView, Replay, RunRecoveryView, InterruptedValidationView, ArtifactView, ValidationView } from './components/evidence-view';
...
export function App() {
  ...
  return <BrowserPresentation browserBox={browserBox} revision={revision} overlay={overlay} onError={setError}><div className="app-shell">
    <header className="topbar"><div className="brand"><strong>Browser Evidence Studio</strong></div><div className="topbar-spacer" />
    ...
    <main className="workspace"><SplitPane key={revision} name="workspace" label="调整工作台与浏览器宽度" initial={Math.min(440, Math.max(320, window.innerWidth * .3)) / window.innerWidth * 100} minFirst="320px" minSecond="400px" first={<aside className="workspace-panel">
      <div className="project-controls"><div className="selector-row"><NativeSelect aria-label="项目" title={project?.objective || project?.name} value={projectId} disabled={!!active} onChange={event => setProjectId(event.target.value)}>{projects.length ? projects.map((entry: any) => <option key={entry.id} value={entry.id}>{entry.name}</option>) : <option value="">选择项目</option>}</NativeSelect><Button size="icon" aria-label="新建项目" title="新建项目" disabled={!!active} onClick={() => setOverlay('project')}><Plus /></Button></div>
      <div className="selector-row"><NativeSelect aria-label="登录环境" value={profileId} disabled={!!active} onChange={event => setProfileId(event.target.value)}>{profiles.length ? profiles.map((profile: any) => <option key={profile.id} value={profile.id}>{profile.name}</option>) : <option value="">选择登录环境</option>}</NativeSelect><Button size="icon" aria-label="添加环境" title="添加环境" disabled={!projectId || !!active} onClick={() => setOverlay('profile')}><Plus /></Button></div>
      <div className="run-controls">{active ? <><Badge value={active.capture} /><Button disabled={!!busy} onClick={() => perform(async () => { await call('seal', {}, '运行已封存，可从存档重新打开。'); await loadHistory(active.id); })}>结束并封存</Button></> : <Button variant="default" className="primary full" disabled={!projectId || !profileId || !!busy} onClick={() => perform(() => call('startRun', { projectId, profileId, url, kind: 'demonstrate' }, '录制已启动。'))}>开始录制</Button>}</div></div>
      ...
      {active?.handoff?.status === 'waiting' && <div className="handoff-banner"><div><strong>等待你完成页面操作</strong><p>{active.handoff.instructions || '完成页面要求后，明确交还控制。'}</p><small>等待与超时不表示成功；交还后会验证实际页面状态。</small></div><Button variant="default" className="primary" disabled={!!busy} onClick={() => perform(() => call('releaseHuman', {}, '已提交交还请求；按真实完成条件判断。'))}>交还控制</Button></div>}
      ...
      <div className="evidence-strip"><div><span>{active ? `Run ${active.id.slice(0, 12)} · ${workspaceCheckpoints.length} 个已加载 checkpoint` : '连续录制与保存点分别记录'}</span></div><Button disabled={!workspaceRunId} onClick={() => perform(() => openHistory(workspaceRunId))}>打开证据时间线</Button><Button disabled={!workspaceRunId} onClick={() => perform(async () => { const result = await call('replay', { runId: workspaceRunId, pageId: selectedPage?.pageId }); setReplay(result); setOverlay('replay'); })}>DOM 回放</Button></div>

    </aside>} second={<section className="browser-workspace"><form className="addressbar" onSubmit={event => { event.preventDefault(); if (active) perform(() => call('navigate', { url })); }}><span className="address-prefix">URL</span><Input ref={addressInput} aria-label="浏览器地址" value={url} onChange={event => setUrl(event.target.value)} placeholder="https://" /><Button disabled={!active || !!busy || active.controller === 'agent'}>前往</Button><Button type="button" className="quiet" disabled={!!busy} onClick={() => perform(synthetic)}>合成站点</Button></form>
        ...
        <div className={`native-browser ${!active ? 'inactive' : ''}`} ref={browserBox}>{!active && <div className="browser-empty"><p>选择项目和登录环境后开始录制</p><span className="muted">浏览器页面将在此处显示</span></div>}</div>
