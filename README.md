...
**当前为可运行的开发版本。** M0–M5 核心流程已实现并有合成通过记录；2026-09-24 类型检查与 Vitest **155/155** 测试通过，源码和发行包的桌面矩阵分别通过。最新 Windows 应用包的 30 分钟固定负载完成 **1,800/1,800 次动作、零漏槽**，独立证据重开和恢复矩阵通过，同版 ZIP 已生成。页面私有内存仍持续增长，原因尚未确认；Windows 物理鼠标体验、技能完整接续和真实业务验收也尚待完成，范围见 [验证记录](docs/verification.md) 与 [进度与接续](docs/progress.md)。
