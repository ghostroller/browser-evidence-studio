...
最新一批收尾为 writer 所有权核验、验收生命周期持久化/重建及恢复界面；恢复保留证据并从新 run 重试，不恢复旧 worker 栈或人工等待。退出诊断和强杀重开矩阵已纳入统一桌面入口。后续仍需长历史续读/回放定位、示范基线持久关联、使用新诊断完成 20 分钟复验，再处理技能接续和发行验收；实际 30 分钟全负载及性能门槛未验收。

用户随后将前端重构提前，先完成 [重构文档](frontend-refactor.md) 再实施 shadcn 组件、默认亮色/暗色主题、紧凑左右分栏及可调整的信息区。此项保留既有业务协议和证据格式；原生遮挡、尺寸同步与窗口退出按实际桌面场景回归，最终结果集中在 [verification.md](verification.md)。上面的长历史、长测及发行事项仍需后续独立完成。
