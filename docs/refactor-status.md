...
**当前进度（2026-09-26 Q2 后）**：Q1 `a0fbbd3` 与 Q2 的生产资源/播放修复已完成，Q2 真实完整桌面矩阵、录制离线、系统专项、54 文件/326 项全量单元、类型检查与构建通过；见 [Q2 交接](refactor-handoffs/Q2-20260926.md)。正在接续 Q3 固定任务交接；Q3 独立模块尚未接入 UI/API，不能称 Q3 已通过。新架构长测、安装版、新 Agent、真人业务仍待验收。R1–R8 实现提交 `f62e5f4` 及下方旧暂停/A–G 派发记录仅作历史，不重复启动或重置工作树。
...
| T03 | T00/T02 | A/G | implementing | Q2 生产录制与离线 ReplayHost 实测晚加载、重复 URL、跨源、cache、Service Worker、`data:`/blob；跨源 iframe DOM 和长档仍不支持/未验收 |
| T04 | T02/T03 | A/G | implementing | 真实 React UI 原生连续播放、准备中暂停、前后定位与前台多文档回归通过；不同物理 DPI/zoom 和长档尚未验收 |
| T05 | T00/T02/T04 | A | implementing | 生产源属性、移动节点保留元数据、同源 frame/open shadow/SVG 与显示采样专项通过；能力边界仍待完整覆盖 |
| T06 | T00 | B | implementing | 固定资料、冲突、版本 hash 已集成；真实 UI 卡片/需求/字段/注释/固定版本查看及派生通过，完整交付仍待验收 |
| T07 | T04/T05/T06 | D | implementing | 真实 React UI 完成卡片、新需求就地关联、字段、注释新增/修改、固定版本查看和派生；更广交互仍待验收 |
| T08 | T05/T06/T07 | D | implementing | 隔离历史选择经原生鼠标、Escape、蒙版和选择回执实跑；源节点选择与注释/字段不串绑；更广焦点边界待验收 |
| T09 | T00/T01 | C | implementing | partial/cancel/timeout 真桌面专项与 G 固定版本真实 worker 通过；最终业务及 UI 待验收 |
| T10 | T00/T09 | C | implementing | 耐久批次及崩溃发现已接入；坏目录逐项诊断、好数据保留和完成后只读审计经回归/UI 通过；最终独立业务交付仍待验收 |
| T11 | T06/T09/T10 | F+D | implementing | 双实体正确值通过、错误值失败、partial 保留数据；真实结果 UI 显示完整/部分及坏目录旁的已提交结果 |
| T12 | T01/T06/T09；历史读接 A | E | implementing | state/jobs 及读取范围、撤销和旧一次授权入口统一；真实 UI 签发/撤销与 HTTP 拒读通过，剩余接口和交付仍待验收 |
| T13 | T06/T11/T12 | E | implementing | 固定执行/报告门面已接入；完整交接导出、技能、差异与订阅尚未完成 |
| T14 | 持续 | S0/G | implementing | Q2 54 文件/326 项（含 Q3 独立模块 3 项）、类型检查/构建、完整未跳 UI 桌面及恢复矩阵、录制离线和系统专项通过；新架构长测未跑 |
| T15 | 全部接入 | G | implementing | 已开始真实系统集成；新安装包、独立交付、新 Agent/真人及完整长测尚未验收 |

以下为较早 A–G 执行记录；当前接续按 R1–R8 交接和用户本次限界要求执行。历史 S0 handoff 保持原样，不由旧派发文本自动展开新任务。
