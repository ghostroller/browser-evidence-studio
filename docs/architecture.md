...
2026-09-24 修正可信窗口导航策略：允许与 `uiUrl` 完全一致的文档刷新，使 Forge/Vite 的首次 HMR 与模块失效恢复能够提交；同源其他路径、查询参数变化和外部地址继续拒绝。刷新真正提交后仍清理旧显示状态，并等待新文档上报有效浏览器边界；被拒导航不清理旧状态。启动加载失败时在生命周期日志追加 `ui-startup-failed`，只保存文档/DOM/边界计数、就绪状态、preload 失败标记及 renderer 退出码，不记录 URL、DOM、业务正文或凭据。
...
操作连接终止区分 host、worker 主动关闭与底层 transport 通知。Puppeteer 在异常清理中先 disconnect，再上报原始异常；主进程立即关闸，但给 worker 最多 1 秒返回错误或退出，不能用笼统断线覆盖它。无终态时明确失败；真实远端断线立即失败，取消或已经确定的首个错误不被迟到 worker error 覆盖。报告保存 `errorSource`、有界 `errorStack`、`operationTransportClose`，同时追加 `operation-transport-closed` 事件。底层提供 local/remote/error、时间、可取得的关闭码、原因和错误标识；未收到关闭帧或只有 error 时不猜 peer 原因。关闭前只记录最多 16 个协议方法名（每项 128 字符），不保存参数；原因/错误消息限 512 字符并处理常见凭据/地址。关闭通知只发一次。

业务脚本每个数据集采集完成并具备来源后应立即 await emitData；后续异常不会撤销已落盘材料。部分数据保留不代表完整执行或需求通过，未完成的阶段不能以空列表补交。当前京东示例按六个阶段提交，每个数据集仍仅提交一次。
