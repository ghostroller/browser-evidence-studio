...
评估日期：2026-09-23。本文保留当时的只读选型快照；下文的测试数量、依赖兼容判断和“拟采用”等说法只描述评估时状态。评估阶段没有为 Vitest 安装依赖、修改测试/构建配置或运行测试；随后实施的[路径别名迁移](import-alias-plan.md)也没有包含 Vitest。

实施状态（2026-09-24）：普通单元与模块测试按本建议迁移到 Vitest，使用独立配置；真实 Electron 桌面入口和显式启动的原生 Node 子进程继续独立运行。实施后的命令、版本、结果和未覆盖范围以 [verification.md](verification.md) 为准。评估时的兼容性声明不能代替实测。
...
评估时建议日常入口保持 npm test（一次性执行）并补一个 watch 入口；test:integration/test:desktop/test:soak 保持真实桌面职责。这一段是当时的提案，当前脚本以 [package.json](../package.json) 为准。
