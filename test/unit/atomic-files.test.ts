...
test('Windows transient replacement failures reuse one synced temporary file while preserving the old target', { skip: process.platform !== 'win32' }, async () => {
  const f = await fixture(), rename = fs.rename.bind(fs), open = fs.open.bind(fs);
  const sources: string[] = []; let syncs = 0, temporaryCreates = 0;
  try {
    vi.spyOn(fs, 'open').mockImplementation(async (file: Parameters<typeof fs.open>[0], flags: Parameters<typeof fs.open>[1], mode?: Parameters<typeof fs.open>[2]) => {
      const handle = await open(file, flags, mode);
      if (flags === 'wx') {
        temporaryCreates++;
        const sync = handle.sync.bind(handle);
        vi.spyOn(handle, 'sync').mockImplementation(async () => { await sync(); syncs++; });
      }
      return handle;
    });
    vi.spyOn(fs, 'rename').mockImplementation(async (source: Parameters<typeof fs.rename>[0], destination: Parameters<typeof fs.rename>[1]) => {
...
test('Windows permanent sharing failure stops within its retry budget and removes only its temporary file', { skip: process.platform !== 'win32' }, async () => {
...
test('non-retryable replacement failure is returned immediately with the confirmed target unchanged', async () => {
  const f = await fixture(), failure = Object.assign(new Error('Synthetic invalid replacement'), { code: 'EINVAL' }); let attempts = 0;
  try {
    vi.spyOn(fs, 'rename').mockImplementation(async () => { attempts++; throw failure; });
    await assert.rejects(atomicFile(f.file, f.newBytes), error => error === failure);
