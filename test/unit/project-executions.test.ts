...
describe('fixed material / worker / source host adapter',()=>{
  it('binds real snapshot fingerprints, persists step recording scope, and reads independently verified JSON after restart',async()=>{
    ...
    try{
      ...
      const report=await reopened.assess('project','execution');expect(report.overall).toBe('pass');expect(report.version.verdict).toBe('pass');expect(report.materialStatus).toBe('candidate');
      const page=await reopened.reportItems('project','execution',report.reportId,'requirements',{maxBytes:8192,limit:10});expect(page.items).toHaveLength(1);expect((page.items[0] as any).humanReviews).toEqual([]);expect((page.items[0] as any).sourceVerdict).toBe('pass');
