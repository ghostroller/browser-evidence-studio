...
beforeEach(async () => {
  ...
  materials = new FileMaterialService(root);
...
async function revision() {
  ...
  const updated = await materials.updateDraft(projectId, draft.draftId, 0, content(), 'human');
...
describe('fixed task handoff', () => {
  it('exports a hashed, bounded reference bundle while retaining the exact V1 baseline after V2', async () => {
    ...
    expect(manifest.requirements).toEqual([{ id: 'orders', dataset: 'orders', fieldIds: ['amount'], ruleTypes: ['min-rows'] }]);
    expect(manifest.fields[0].sourcePolicy).toBe('page-displayed');
    expect(manifest.taskSha256).toBe(createHash('sha256').update(task).digest('hex'));
    expect(`${task}\n${JSON.stringify(manifest)}`).not.toMatch(/private-cookie|private-token|private-value|Private customer name/);
